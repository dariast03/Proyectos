...
const EditarPlato = () => {
  ...
  useEffect(() => {
    const fetchPlato = async () => {
      try {
        const response = await fetch(`https://localhost:7263/api/Platos/Buscar/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (imagenPlato) {
      ...
      const uploadResponse = await fetch(`https://localhost:7263/api/Platos/SubirImagen?fileName=${imagenPlato.name}`, {
        method: 'POST',
        body: formData,
      });
    ...
    try {
      const response = await fetch(`https://localhost:7263/api/Platos/Actualizar/${id}`, {
        method: 'PUT',
        headers: {
