...
const Promociones = ({ setIsLoggedIn }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('https://51r87rnm-7263.brs.devtunnels.ms/api/Promociones/Insertar', {
        method: 'POST',
        headers: {
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`https://51r87rnm-7263.brs.devtunnels.ms/api/Promociones/Actualizar/${editingPromocion.iD_Promocion}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
