...
const NotificacionAdmin = () => {
  ...
  useEffect(() => {
    if (platos.length > 0) {
      const fetchPedidos = async () => {
        try {
          const response = await fetch('https://51r87rnm-7263.brs.devtunnels.ms/api/PedidoClientes/Listar');
          const data = await response.json();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('https://51r87rnm-7263.brs.devtunnels.ms/api/Notificaciones/Insertar', {
        method: 'POST',
        headers: {
